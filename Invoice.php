...
class Invoice {
  ...
  const INVOICE_API_BASE_URL = 'api2/';
  const INVOICE_STITCH_CLASS_NAME = 'Invoices';
  ...
  /**
   * Wrapper function to look up Invoices by their ids.
   *
   * @param: $filters
  ...
  public function lookUpId($ids = array(), $detail = TRUE) {
    ...
    $args = array(
      $this::INVOICE_STITCH_CLASS_NAME => array($ids),
    );
    return $this->lookUp($args, $detail);
  }

  /**
   * Utility function to lookup invoices by arguments provided.
   */
  ...
  private function request($action, $arguments = array(), $detail = FALSE) {
    $stitch = $this->stitch;
    $url = $stitch::STITCH_API_BASE_URL . $this::INVOICE_API_BASE_URL;
    $url .= $this->getAPIVersion($action) . '/' . $this::INVOICE_STITCH_CLASS_NAME;
  ...
  public function getArguments($action) {
    ...
    $invoice_values = clone $this;
    unset($invoice_values->stitch);

    if (empty($invoice_values->id)) {
      unset($invoice_values->id);
    }

    $args = array(
      'action' => $action,
      $this::INVOICE_STITCH_CLASS_NAME => array(
        $invoice_values,
      ),
    );
